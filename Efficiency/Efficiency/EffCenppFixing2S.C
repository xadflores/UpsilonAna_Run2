...
//Macro to get Efficiency of 2S vs. pt in pp. 
//run as follows:
//> root -l
//> .L trigEff.C+
//> trigEff()
...
//OniaTree_Ups2SMM_5p02TeV_TuneCUETP8M1_ptUps03_noCUT.root
//Do the following:
...
double pp2S_coefficient =  -0.0180556;
double pp2S_constant = 1.1111;

void EffCenppFixing2S(){
...
//	myTree.SetBranchAddress("Gen_QQ_sign", Gen_QQ_sign, &b_Gen_QQ_sign);
	...
	TH1D  *hMassL1=new TH1D("hMassL1","Reconstructed", 1, 0, 30);

	TH1D  *hgenMassL1=new TH1D("hgenMassL1","Generated", 1, 0, 30);
...
//	TH1D* hEff = new TH1D("Eff", "", 1, 0, 30);
        ...
        ReweightFunctions->GetObject("pp2S", pp2S);
        ReweightFunctions->GetObject("pp2Smc", pp2Smc);
...
//                         if(qq4mom->Pt()<=30){ptReweight = PtReweight(qq4mom, pp2S_coefficient, pp2S_constant);}
                         if(qq4mom->Pt()<=30){ptReweight = (pp2S->Eval(qq4mom->Pt()))/(pp2Smc->Eval(qq4mom->Pt()));}
                         weight = ptReweight;
...
//	TrigEff->SetMarkerColor(2);
	...
	TrigEff->Draw("AP");	// */
	c1->Update();
	c1->SaveAs("EfficiencyVsCenUpsilonpp2S.png");

 TFile* MyFileEff;
          MyFileEff = new TFile("CenppEff2S.root", "Recreate");
  TrigEff->Write();
  MyFileEff->Close();
