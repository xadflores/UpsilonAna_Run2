...
void Raa2S_Workspace(const char* name_pbpb="fitresult.root", const char* name_pp="fitresult_pp.root", const char* name_out="fitresult_combo2.root"){
...
bool doSyst = true;
   ...
   RooRealVar* DR21 = new RooRealVar("DR21","DoubleRatio(#Upsilon (2S))",0.5,0.,1.);
   RooRealVar* leftEdge = new RooRealVar("leftEdge","leftEdge",0);
   RooRealVar* rightEdge = new RooRealVar("rightEdge","rightEdge",1);
   RooGenericPdf step("step", "step", "(@0 >= @1) && (@0 < @2)", RooArgList(*DR21, *leftEdge, *rightEdge));
   ws->import(step);
   ws->factory( "Uniform::flat(DR21)" );
//-- From jpsi crew
/*
   ws->factory( Form("kappa_syst[%f]",1.1555) );
...
*/  
   ...
   //ws->factory( "Gaussian::constr_syst_add(beta_syst_add[0,-5,5],glob_syst_add[0,-5,5],kappa_syst_add)" );
     ...
     ws->factory( Form("kappa_syst[%f]",0.029066) );//0-5
   ...
   RooRealVar* frac2Sv1S_pp = ws->var("R_{#frac{2S}{1S}}_pp");
   RooRealVar* nsig1_hi = ws->var("N_{#varUpsilon(1S)}_hi");
   //cout << frac3Sv1S_pp << endl;
   RooFormulaVar nsig2_hi_modified("nsig2_hi_modified", "(@0+@1)*@2*@3", RooArgList(*DR21,*alphaSyst, *frac2Sv1S_pp,*nsig1_hi));
...
//   ws->var("Centrality")->setConstant(true);
   ...
   ws->var("R_{#frac{3S}{1S}}_hi")->setConstant(true);
...
//   ws->var("N_{#Upsilon(3S)}_pp")->setConstant(true);
   ...
   // ws->Print();
   /////////////////////////////////////////////////////////////////////
   RooAbsReal * pNll = sbHypo.GetPdf()->createNLL( *data,NumCPU(8) );
   RooMinuit(*pNll).migrad(); // minimize likelihood wrt all parameters before making plots
   RooPlot *framepoi = ((RooRealVar *)poi.first())->frame(Bins(10),Range(0.,.2),Title("LL and profileLL in DR21"));
