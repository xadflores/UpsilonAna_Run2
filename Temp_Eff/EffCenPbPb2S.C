...
void EffCenPbPb2S(){      // Change function name
        gROOT->Macro("logon.C+");
	...
	double          ptWeight; 
	double          centWeight;
	double          ptBin[nPtBin] = {1.5,4.5};   //  RapBin
	double          ptBinErr[nPtBin] = {1.5,1.5};  // RapBinErr
	float           CenBin[nCenBin] = {5,15,30,50,70,90,110,130,150};
        float           CenBinErr[nCenBin] = {5,5,10,10,10,10,10,10,10};
	float    	CenBinEdges[nCenBin+1] = {0,10,20,40,60,80,100,120,140,160};
        double          ptReweight=0.0;
	int             CenMin[nCenBin] = {0,10,20,40,60,80,100,120,140};
	int             CenMax[nCenBin] = {10,20,40,60,80,100,120,140,160};
	...
	TH1I  *RecoEvents=new TH1I("RecoEvents","Reconstructed", nCenBin,CenBinEdges);

	TH1I  *GenEvents=new TH1I("GenEvents","Generated", nCenBin,CenBinEdges);
	...
	TF1* AA2Smc;
        ...
        AA2S = (TF1*)ReweightFunctions->Get("AA2S");
        AA2Smc= (TF1*)ReweightFunctions->Get("AA2Smc");

        ///try with 1S functions and works ???
        //AA2S = (TF1*)ReweightFunctions->Get("AA1S");
        //AA2Smc= (TF1*)ReweightFunctions->Get("AA1Smc");
	...
	for (Long64_t jentry=0; jentry<nentries; jentry++){
		...
		for (int iQQ=0; iQQ<Reco_QQ_size;iQQ++){
			...
			if(TMath::Abs(qq4mom->Rapidity())<2.4 && qq4mom->Pt()<30){
					if(recoPass == 1 && PtCutPass ==1 && MassCutPass == 1){RecoEvents->Fill(Centrality, weight);}
			}
		...
		for (int iQQ=0; iQQ<Gen_QQ_size;iQQ++){
			...
			MassCutPass = MassCut(g_qq4mom, m2S_low, m2S_high);
			...
			//weights only needed for PbPb
			 ...
			 //reweight from functions
                         ptReweight = (AA2S->Eval(genPt))/(AA2Smc->Eval(genPt));
			...
			if(TMath::Abs(g_qq4mom->Rapidity())<2.4 && g_qq4mom->Pt()<30){
			if(acceptMu == 1 && PtCutPass == 1 && MassCutPass == 1){GenEvents->Fill(Centrality, weight);}
			}
...
TGraphAsymmErrors *TrigEff = new TGraphAsymmErrors(nCenBin);
...
TrigEff->GetYaxis()->SetTitle("Efficiency[#varUpsilon(2S)]_{PbPb}");
TrigEff->GetXaxis()->SetTitle("Centrality");
TrigEff->GetYaxis()->SetRangeUser(0,1);
TrigEff->GetXaxis()->SetRangeUser(0.0, 160.0);
TrigEff->GetXaxis()->CenterTitle();
TrigEff->GetYaxis()->CenterTitle();

TrigEff->Draw("AP");
c1->Update();
c1->SaveAs("EfficiencyVsCentralityUpsilonPbPb2S.png");
...
TFile* MyFileEff;
MyFileEff = new TFile("CenPbPbEff2S.root", "Recreate");
TrigEff->Write();
MyFileEff->Close();

        for (Int_t i = 0; i < (nCenBin); i++){
        cout << TrigEff->Eval(CenBin[i]) << " , - " << TrigEff->GetErrorYlow(i) << " , + " << TrigEff->GetErrorYhigh(i) << endl;
        }
...
double GetWeight(int numTree){
  //double weight[6] = {3.10497,4.11498,2.2579,1.2591,0.567094,0.783399};
  double weight[6] = {5.89168,9.08207,3.106,1.10018,0.534916,0.776183};
  return weight[numTree];
}
