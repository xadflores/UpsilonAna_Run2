...
void makeWorkspace2015(RooWorkspace& ws, const TString FileName, struct InputOpt opt, TH1F* hDataOS,bool doPeriph){
 
  std::string finput(FileName);
  TFile *f = new TFile(finput.c_str());
  TTree* theTree       = (TTree*)f->Get("myTree"); // OS --- all mass
 
  RooRealVar* mass       = new RooRealVar("invariantMass","#mu#mu mass", opt.dMuon.M.Min, opt.dMuon.M.Max, "GeV/c^{2}");	
  RooRealVar* dimuPt     = new RooRealVar("dimuPt","p_{T}(#DiMuon)",0,100,"GeV/c");
  ...
  theTree->SetBranchStatus("runNb", 1);
  theTree->SetBranchStatus("Centrality",1);
  theTree->SetBranchStatus("HLTriggers", 1);
  theTree->SetBranchStatus("Reco_QQ_size",1);
  theTree->SetBranchStatus("Reco_QQ_sign", 1);
  theTree->SetBranchStatus("Reco_QQ_4mom", 1);
  theTree->SetBranchStatus("Reco_QQ_mupl_4mom", 1);
  theTree->SetBranchStatus("Reco_QQ_mumi_4mom", 1);
  theTree->SetBranchStatus("Reco_QQ_trig", 1);
  theTree->SetBranchStatus("Reco_QQ_VtxProb",1);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      for (int i=0; i<Reco_QQ_size; i++) {
  	 ...
  	 //pp trig         
         if(!opt.isPbPb && ((HLTriggers&1)==1 && (Reco_QQ_trig[i]&1)==1)){data0->add(cols);}
         //pbpb DoubleMu0 trig PD and centrality check
	 if(opt.isPbPb && doPeriph==false && centrality>=opt.Centrality.Start && centrality<opt.Centrality.End && (((HLTriggers&1)==1 && (Reco_QQ_trig[i]&1)==1) || ((HLTriggers&2)==2 && (Reco_QQ_trig[i]&2)==2) || ((HLTriggers&4)==4 && (Reco_QQ_trig[i]&4)==4))){data0->add(cols);}
         //pbpb Periph trig PD and centrality check
	 if(opt.isPbPb && doPeriph  && centrality>=opt.Centrality.Start && centrality<opt.Centrality.End && (((HLTriggers&128)==128 && (Reco_QQ_trig[i]&128)==128) || ((HLTriggers&256)==256 && (Reco_QQ_trig[i]&256)==256))){data0->add(cols);}


	///not super impoertant
         if ( 
             (Reco_QQ_sign[i]==0) && 
